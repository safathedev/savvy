import { CALENDAR_CATEGORIES, CalendarCategory } from "@/constants/calendar-types";
...
interface AgendaViewProps {
    events: any[]; // Using any for now to match main file, should be typed
    selectedDate: Date;
    ...
    if (sortedEvents.length === 0) {
        return (
            <View style={s.emptyContainer}>
                ...
                <Text style={s.emptySubtitle}>Time for a coffee? ☕</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView style={s.container} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
            {sortedEvents.map((event, index) => {
                const categoryConfig = CALENDAR_CATEGORIES[event.type as CalendarCategory] || CALENDAR_CATEGORIES.other;
                const [h, m] = (event.time || "00:00").split(':');
                    ...
                    <Animated.View key={event.id} entering={FadeInDown.delay(index * 100).springify()}>
                        <Pressable onPress={() => onEditEvent(event)} style={s.card}>
                            <View style={[s.timeCol, { borderRightColor: categoryConfig.color }]}>
                                <Text style={s.startTime}>{event.time}</Text>
